...
import models.WAETwoLevel.ModelMaps as ModelMaps
...
class Model():
    ...
    def generate_modules(self, batch):
        ...
        self.PreEnc = ModelMaps.PreEnc({**self.config, 'data_properties': batch['observed']['properties']})        
        self.PostGen = ModelMaps.PostGen({**self.config, 'data_properties': batch['observed']['properties']})        
    ...
    def inference(self, batch, additional_inputs_tf):
        ...
        # self.cri_cost = tf.reduce_mean(tf.nn.relu(self.sample_distance_function(self.input_sample, self.reconst_sample)-2))

        # self.reconst_param = self.full_reconst_param
        # self.reconst_dist = self.full_reconst_dist
        # self.reconst_sample = self.full_reconst_sample
