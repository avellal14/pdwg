...
class Model():
    ...
    def inference(self, batch, additional_inputs_tf):
        ...
        if self.compute_all_regularizers or \
           ...
           'Coupling Gradient Direction' in self.config['critic_reg_mode'] or \
           ...
           'Coupling Lipschitz' in self.config['critic_reg_mode']:
            ...
            try: 
                self.coupling_line_sample_param['image'] = tf.stop_gradient(\
                                                           self.uniform_sample_a[:, np.newaxis, :, np.newaxis, np.newaxis]*self.reconst_sample['image']+\
                                                           (1-self.uniform_sample_a[:, np.newaxis, :, np.newaxis, np.newaxis])*self.input_sample['image'])
            except: 
                self.coupling_line_sample_param['flat'] = tf.stop_gradient(\
                                                          self.uniform_sample_a[:, np.newaxis, :]*self.reconst_sample['flat']+\
        ...
        self.critic_real_raw = self.Critic.forward(self.input_sample)
        self.critic_rec_raw = self.Critic.forward(self.reconst_sample)

        self.critic_real = self.critic_real_raw #+ self.sample_distance_function(self.input_sample, self.reconst_sample)
        self.critic_rec = self.critic_rec_raw - self.sample_distance_function(self.input_sample, self.reconst_sample)
        
        self.mean_critic_real = tf.reduce_mean(self.critic_real)
        self.mean_critic_rec = tf.reduce_mean(self.critic_rec)

        self.coupling_line_sample = self.reconst_sample
        self.critic_coupling_line = self.critic_rec
        self.mean_critic_reg = tf.reduce_mean(self.critic_coupling_line)

        try: 
            self.coupling_grad = tf.gradients(tf.reduce_sum(self.critic_rec_raw), [self.reconst_sample['image']])[0]
            self.coupling_grad_norm_sq = tf.reduce_sum(self.coupling_grad**2, axis=[-1,-2,-3], keep_dims=False)[:,:,np.newaxis]
        except: 
            self.coupling_grad = tf.gradients(tf.reduce_sum(self.critic_rec_raw), [self.reconst_sample['flat']])[0]
            self.coupling_grad_norm_sq = tf.reduce_sum(self.coupling_grad**2, axis=[-1,], keep_dims=True)
            
        try: 
            self.coupling_line_grad = tf.gradients(tf.reduce_sum(self.critic_coupling_line), [self.coupling_line_sample['image']])[0]
            ...
            self.coupling_line_grad = tf.gradients(tf.reduce_sum(self.critic_coupling_line), [self.coupling_line_sample['flat']])[0]
        ...
        try: 
            ...
            self.desired_slope_vector_norm = helper.safe_tf_sqrt(tf.reduce_sum(self.desired_slope_vector**2, axis=[-1,-2,-3], keep_dims=False))[:,:,np.newaxis,np.newaxis,np.newaxis]
            self.desired_slope = tf.stop_gradient(self.desired_slope_vector/self.desired_slope_vector_norm)
        except: 
            self.desired_slope_vector = self.input_sample['flat']-self.reconst_sample['flat']
            self.desired_slope_vector_norm = helper.safe_tf_sqrt(tf.reduce_sum(self.desired_slope_vector**2, axis=[-1,], keep_dims=False))[:,:,np.newaxis]
            self.desired_slope = tf.stop_gradient(self.desired_slope_vector/self.desired_slope_vector_norm)
        ...
        self.coupling_grad_norm_penalties = self.coupling_grad_norm_sq
        self.mean_coupling_grad_norm_penalties = tf.reduce_mean(self.coupling_grad_norm_penalties)
        self.cri_reg_cost = self.mean_coupling_grad_norm_penalties
        ...
        # #     if self.compute_all_regularizers or \
        # #        'Coupling Gradient Vector' in self.config['critic_reg_mode'] or \
        # #        'Coupling Gradient Direction' in self.config['critic_reg_mode']:
        ...
        # if self.compute_all_regularizers or 'Coupling Gradient Direction' in self.config['critic_reg_mode']:
        #     if self.input_sample['image'] is not None:                
        #         self.coupling_line_grad_direction_penalties = -tf.reduce_sum(((self.coupling_line_grad/self.coupling_line_grad_norm[:,:,:,np.newaxis,np.newaxis])*self.desired_slope), axis=[-1,-2,-3], keep_dims=False)[:,:,np.newaxis]
        #     else:
        #         self.coupling_line_grad_direction_penalties = -tf.reduce_sum(((self.coupling_line_grad/self.coupling_line_grad_norm)*self.desired_slope), axis=[-1,], keep_dims=False)[:,:,np.newaxis]

        #     self.mean_coupling_line_grad_direction_penalties = tf.reduce_mean(self.coupling_line_grad_direction_penalties)
        #     if 'Coupling Gradient Direction' in self.config['critic_reg_mode']: 
        #         print('Adding Coupling Gradient Direction Penalty')
        #         self.cri_reg_cost += self.mean_coupling_line_grad_direction_penalties
