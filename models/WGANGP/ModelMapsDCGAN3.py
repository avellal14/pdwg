	...
	def forward(self, x, noise=None, name = ''):
		with tf.variable_scope("Decomposer", reuse=self.constructed):
			...
			for i in range(self.n_steps):
				...
				curr_hidden = tf.layers.dense(inputs = curr_input, units = int(hidden_sizes[i]), activation = self.activation_function)
				curr_reconst = tf.layers.dense(inputs = curr_hidden, units = x_batched_inp_flat.get_shape().as_list()[-1], activation = None)
			...
			self.constructed = True
			return decomposition_out, deterioration_out, decomposition_concat_out, deterioration_concat_out, reconstruction_cost
	...
	def forward(self, x, noise=None, name = ''):
		with tf.variable_scope("Encoder", reuse=self.constructed):
			...
			if len(self.config['data_properties']['image']) > 0:								
				...
				x_batched_inp_flat = tf.reshape(x['image'], [-1,  n_image_size*n_output_channels])
