...
class Model():
    ...
    def inference(self, batch, additional_inputs_tf):
        ...
        Euclidean_flow_class = PiecewisePlanarScalingFlow
        self.pre_flow_object = transforms.SerialFlow([\
                                                  transforms.Euclidean_flow_class(input_dim=self.config['n_latent'], parameters=self.wolf_param_list[0]), 
                                                  transforms.SpecificRotationFlow(input_dim=self.config['n_latent']), 
                                                  transforms.Euclidean_flow_class(input_dim=self.config['n_latent'], parameters=self.wolf_param_list[1]), 
                                                  transforms.SpecificRotationFlow(input_dim=self.config['n_latent']), 
                                                  ])

        self.flow_object = transforms.SerialFlow([\
                                                  transforms.Euclidean_flow_class(input_dim=self.config['n_latent'], parameters=self.flow_param_list[0]), 
                                                  transforms.SpecificRotationFlow(input_dim=self.config['n_latent']), 
                                                  transforms.Euclidean_flow_class(input_dim=self.config['n_latent'], parameters=self.flow_param_list[1]), 
