...
class Model():
    ...
    def inference(self, batch, additional_inputs_tf):
        ...
        if self.config['divergence_mode'] == 'GAN' or self.config['divergence_mode'] == 'NS-GAN' or self.config['divergence_mode'] == 'WGAN-GP':
        ...
        elif self.config['divergence_mode'] == 'MMD':
            ...
            dist_per_direction = helper.wasserstein_metric_1D_gaussian_columns(self.posterior_random_projections, self.prior_random_projections)
        ...
        if self.config['divergence_mode'] == 'GAN' or self.config['divergence_mode'] == 'NS-GAN':
            self.div_cost = -(tf.reduce_mean(tf.log(tf.nn.sigmoid(self.div_prior)+1e-7))+tf.reduce_mean(tf.log(1-tf.nn.sigmoid(self.div_posterior)+1e-7)))
        elif self.config['divergence_mode'] == 'FLOW-MMD':
            self.div_cost = - self.MMD_transformed
        ...
        elif self.config['divergence_mode'] == 'NS-GAN': 
            self.enc_reg_cost = -tf.reduce_mean(tf.log(tf.nn.sigmoid(self.div_posterior)+1e-7))
