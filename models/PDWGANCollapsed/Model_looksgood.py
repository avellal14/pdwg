...
#                              'encoder_mode': 'UnivApprox', 'divergence_mode': 'MMD', 'dual_dist_mode': 'Coupling', 
...
#                              # 'critic_reg_mode': ['Coupling Gradient Vector',], 'cri_reg_strength': 100, 'lambda_mix': 0}
#                              'critic_reg_mode': ['Coupling Gradient Vector',], 'cri_reg_strength': 10, 'lambda_mix': 1}
...
class Model():
    ...
    def inference(self, batch, additional_inputs_tf):
        ...
        self.critic_real = self.Critic.forward(self.input_sample)-self.sample_distance_function(self.input_sample, self.input_sample)
        self.critic_rec = self.Critic.forward(self.reconst_sample)-self.sample_distance_function(self.input_sample, self.reconst_sample)

        # self.critic_real = self.sample_distance_function(self.input_sample, self.input_sample)
        # self.critic_rec = self.sample_distance_function(self.input_sample, self.reconst_sample)
        ...
        if self.compute_all_regularizers or \
           ...
           'Coupling Lipschitz' in self.config['critic_reg_mode']:
            ...
            # self.critic_coupling_line = self.Critic.forward(self.coupling_line_sample)+self.sample_distance_function(self.input_sample, self.reconst_sample)
            # self.critic_coupling_line = self.sample_distance_function(self.input_sample, self.coupling_line_sample)
