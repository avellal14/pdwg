...
class Model():
    ...
    def compute_MMD(self, sample_batch_1, sample_batch_2, mode='His'):
        if mode == 'Mine':
            ...
            # for scale in [.01, .02, .05, 0.1, 0.2, 0.5, 1.0]:
                C = Cbase * scale
    ...
    def sum_div(self, div_func, batch_input):
    ...
    def stable_div_expanded(self, div_func, batch_input, batch_rand_dirs_expanded, mode='max', projection_dim=10):        
    ...
    def inference(self, batch, additional_inputs_tf):
        ...
        self.uniform_sample_2 = self.uniform_dist.sample()
        self.bernoulli_dist = distributions.BernoulliDistribution(params = self.uniform_sample_2, b_sigmoid=False)
        # self.bernoulli_sample = self.bernoulli_dist.sample(b_mode=False)
        self.bernoulli_sample = tf.stop_gradient(tf.cast(self.uniform_sample_2 > 0.5, tf.float32))

        self.reverse_input_sample = {'flat': None, 'image': None}
        self.reverse_input_sample['image'] = tf.reverse(self.input_sample['image'], [0])
        self.bernoulli_input_sample = {'flat': None, 'image': None}        
        self.bernoulli_input_sample['image'] = self.bernoulli_sample[:, :, np.newaxis, np.newaxis, np.newaxis]*self.input_sample['image']+(1-self.bernoulli_sample[:, :, np.newaxis, np.newaxis, np.newaxis])*self.reverse_input_sample['image'] 
        ...
        self.reversed_posterior_latent_code_expanded = tf.reverse(self.posterior_latent_code_expanded, [0]) 
        self.reversed_posterior_latent_code = self.reversed_posterior_latent_code_expanded[:,0,:]
        self.interpolated_posterior_latent_code_expanded = (1-self.uniform_sample_2[:,:,np.newaxis])*self.posterior_latent_code_expanded+self.uniform_sample_2[:,:,np.newaxis]*self.reversed_posterior_latent_code_expanded
        self.interpolated_posterior_latent_code = self.interpolated_posterior_latent_code_expanded[:,0,:]

        self.posterior_latent_code_contract_expanded = self.Encoder.forward(self.input_sample, noise=self.epsilon_contract)
        self.posterior_latent_code_contract = self.posterior_latent_code_contract_expanded[:,0,:]
        self.posterior_latent_code_expanded_2 = self.Encoder.forward(self.input_sample, noise=self.epsilon_2)
        ...
        self.bernoulli_reconst_param = self.Generator.forward(self.interpolated_posterior_latent_code_expanded) 
        self.bernoulli_reconst_dist = distributions.ProductDistribution(sample_properties = batch['observed']['properties'], params = self.bernoulli_reconst_param)
        self.bernoulli_reconst_sample = self.bernoulli_reconst_dist.sample(b_mode=True)

        self.separator_cost_posterior = self.Separator.forward(self.posterior_latent_code[:,np.newaxis,:], noise=self.epsilon)
        ### Primal Penalty
        ...
        if self.config['divergence_mode'] == 'MMD-DENOISE':
            self.MMD = self.compute_MMD(self.posterior_latent_code, self.prior_dist.sample())
            # self.separator_cost_posterior = self.Separator.forward(self.posterior_latent_code[:,np.newaxis,:], noise=self.epsilon)
            self.enc_reg_cost = self.MMD+self.config['enc_inv_MMD_strength']*self.separator_cost_posterior
        if self.config['divergence_mode'] == 'MMD-DECOMPOSED': 
            ...
            n_decomposer_steps = len(self.all_hidden_posterior)
            self.MMD_reconst_list = []
            self.MMD_residual_list = []
            self.MMD_hidden_list = []
            for i in range(n_decomposer_steps):
                curr_reconst_MMD = self.compute_MMD(self.decomposition_out_posterior[:, i, :], self.decomposition_out_prior[:, i, :])
                curr_residual_MMD = self.compute_MMD(self.deterioration_out_posterior[:, i, :], self.deterioration_out_prior[:, i, :])
                curr_hidden_MMD = self.compute_MMD(self.all_hidden_posterior[i], self.all_hidden_prior[i])
            ...
            self.MMD_last_residual = self.compute_MMD(self.decomposition_out_posterior[:, -1, :], self.decomposition_out_prior[:, -1, :])
        ...
        if self.config['divergence_mode'] == 'NEW-MMD': 
            ...
            self.MMD = self.compute_MMD(tf.concat([self.posterior_latent_code, transformed_batch_input_inverse[0,:,:]], axis=1), 
        ...
        if self.config['divergence_mode'] == 'DIFF-MMD2':
            posterior_latent_diffs = self.circular_shift(self.posterior_latent_code)-self.circular_shift(self.circular_shift(self.posterior_latent_code)) 
            prior_latent_diffs = self.prior_dist.sample()-self.prior_dist.sample()
            self.MMD = self.compute_MMD(tf.concat([self.posterior_latent_code, posterior_latent_diffs], axis=1), tf.concat([self.prior_dist.sample(), prior_latent_diffs], axis=1))
            self.enc_reg_cost = self.MMD
        if self.config['divergence_mode'] == 'DIFF-MMD':
            ...
            self.MMD = self.compute_MMD(tf.concat([self.posterior_latent_code, self.posterior_latent_code_diff], axis=1), tf.concat([prior_latent_code_1, prior_latent_code_diff], axis=1))
            self.enc_reg_cost = self.MMD
        if self.config['divergence_mode'] == 'SUBSET-MMD': # assumes normal distributed prior
            ...
            for subset_size in subset_sizes:
                ...
                self.MMD += self.compute_MMD(batch_input_shuffled[:, :subset_size], self.posterior_latent_code[:, :subset_size])
            self.enc_reg_cost = self.MMD     
        if self.config['divergence_mode'] == 'SUM-MMD':
            self.Inv_MMD = self.sum_div(self.compute_MMD, self.posterior_latent_code)
            self.MMD = self.compute_MMD(self.posterior_latent_code, self.prior_dist.sample())
        ...
        print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! MMD only positive !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
        self.enc_reg_cost = tf.nn.relu(self.enc_reg_cost)
        ...
        self.div_cost = self.separator_cost_posterior
        ...
        self.OT_interpolate_primal = self.sample_distance_function(self.bernoulli_input_sample, self.bernoulli_reconst_sample)

        self.mean_OT_primal = tf.reduce_mean(self.OT_primal)
        self.mean_OT_interpolate_primal = tf.reduce_mean(self.OT_interpolate_primal) 
        ...
        if b_use_timer:
            ...
            self.mean_POT_primal = (self.mean_OT_primal+self.mean_OT_interpolate_primal)/2+helper.hardstep((self.epoch-float(starttime))/float(timescale)+0.0001)*self.config['enc_reg_strength']*self.enc_reg_cost
        else:
            # self.mean_POT_primal = self.mean_OT_primal+self.config['enc_reg_strength']*self.enc_reg_cost
            self.mean_POT_primal = (self.mean_OT_primal+self.mean_OT_interpolate_primal)/2+self.config['enc_reg_strength']*self.enc_reg_cost
        self.enc_cost = self.mean_POT_primal

        ### Critic
        if self.config['divergence_mode'] == 'MMD-DECOMPOSED':
            self.cri_cost = self.decomposer_cost_posterior

        ### Generator
        self.gen_cost = (self.mean_OT_primal+self.mean_OT_interpolate_primal)/2
