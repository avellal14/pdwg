...
class Model():
    ...
    def generative_model(self, batch, additional_inputs_tf):
        ...
        self.gen_pre_prior_latent_code_expanded = self.gen_pre_prior_latent_code[:,np.newaxis,:]

        self.gen_flow_param = self.FlowMap.forward()
        self.gen_flow_object = transforms.HouseholdRotationFlow(self.gen_flow_param, self.config['n_latent'])
        self.gen_prior_latent_code, _ = self.gen_flow_object.inverse_transform(self.gen_pre_prior_latent_code, tf.zeros(shape=(self.gen_batch_size_tf, 1)))
    ...
    def inference(self, batch, additional_inputs_tf):
        ...
        self.pre_prior_latent_code_expanded = self.pre_prior_latent_code[:,np.newaxis,:]

        # GENERATOR 
        self.flow_param = self.FlowMap.forward()
        self.flow_object = transforms.HouseholdRotationFlow(self.flow_param, self.config['n_latent'])
        self.prior_latent_code, _ = self.flow_object.inverse_transform(self.pre_prior_latent_code, tf.zeros(shape=(self.batch_size_tf, 1)))
        ...
        self.constant_pre_prior_latent_code_expanded = self.constant_pre_prior_latent_code[:, np.newaxis, :]
        ...
        self.posterior_latent_code_expanded, _ = self.Encoder.forward(self.input_sample, noise=self.epsilon)
        ...
        self.transformed_posterior_latent_code_abs = helper.relu_abs(self.transformed_posterior_latent_code)
        self.transformed_posterior_latent_code_sq = self.transformed_posterior_latent_code**2
        self.transformed_posterior_latent_code_abs_means = tf.reduce_mean(self.transformed_posterior_latent_code_abs, axis=0)
        self.transformed_posterior_latent_code_sq_means = tf.reduce_mean(self.transformed_posterior_latent_code_sq, axis=0)
        self.transformed_posterior_latent_code_abs_weighted = (tf.range(1,self.config['n_latent']+1,1, dtype=tf.float32)[np.newaxis, :]/self.config['n_latent'])*self.transformed_posterior_latent_code_abs
        self.transformed_posterior_latent_code_sq_weighted = (tf.range(1,self.config['n_latent']+1,1, dtype=tf.float32)[np.newaxis, :]/self.config['n_latent'])*self.transformed_posterior_latent_code_sq
        self.transformed_posterior_latent_code_abs_weighted_means = tf.reduce_mean(self.transformed_posterior_latent_code_abs_weighted, axis=0)
        self.transformed_posterior_latent_code_sq_weighted_means = tf.reduce_mean(self.transformed_posterior_latent_code_sq_weighted, axis=0)

        self.transformed_posterior_latent_code_norm = tf.reduce_sum(self.transformed_posterior_latent_code_sq, axis=1, keep_dims=True)
        self.transformed_posterior_latent_code_norm_weighted = tf.reduce_sum(self.transformed_posterior_latent_code_sq_weighted, axis=1, keep_dims=True)
        self.shouldbezero_cost = tf.reduce_mean(self.transformed_posterior_latent_code_norm_weighted)
