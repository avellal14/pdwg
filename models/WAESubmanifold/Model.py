...
class Model():
    ...
    def generative_model(self, batch, additional_inputs_tf):
        ...
        self.gen_prior_latent_code = self.PriorExpandMap.forward(self.gen_pre_prior_latent_code_expanded)
    ...
    def inference(self, batch, additional_inputs_tf):
        ...
        self.prior_latent_code = self.PriorExpandMap.forward(self.pre_prior_latent_code_expanded)
        ...
        self.constant_prior_latent_code = self.PriorExpandMap.forward(self.constant_pre_prior_latent_code_expanded)
        ...
        self.pre_posterior_latent_code_expanded, _ = self.Encoder.forward(self.input_sample, noise=self.epsilon)
        self.pre_posterior_latent_code = self.pre_posterior_latent_code_expanded[:,0,:]

        self.nball_param = tf.concat([self.pre_posterior_latent_code, 0.1*tf.ones(shape=(self.batch_size_tf, 1))], axis=1)
        self.nball_dist = distributions.UniformBallDistribution(params=self.tiny_perturb_param) 
        self.posterior_latent_code = self.nball_dist.sample()
        self.posterior_latent_code_log_pdf = self.nball_dist.log_pdf(self.posterior_latent_code)
        ...
        self.interpolated_pre_posterior_latent_code = helper.interpolate_latent_codes(self.posterior_latent_code, size=self.batch_size_tf//2)
        self.interpolated_posterior_latent_code_collapsed = self.PriorExpandMap.forward(tf.reshape(self.interpolated_pre_posterior_latent_code, [-1, 1, self.interpolated_pre_posterior_latent_code.get_shape().as_list()[-1]]))
        self.interpolated_posterior_latent_code = tf.reshape(self.interpolated_posterior_latent_code_collapsed, [-1, *self.interpolated_pre_posterior_latent_code.get_shape().as_list()[-2:]])
        self.interpolated_obs = self.Generator.forward(self.interpolated_posterior_latent_code) 

        self.reconst_param = self.Generator.forward(self.pre_posterior_latent_code_expanded) 
        ...
        ### Primal Penalty
        self.enc_reg_cost = helper.compute_MMD(self.pre_prior_latent_code, self.pre_posterior_latent_code)
