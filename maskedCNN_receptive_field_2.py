...
for curr_n_layers in range(1,n_layers+1):
    ...
    plt.imsave(str(Path.home())+'/masked_CNN_ims/cone_masked_'+str(curr_n_layers)+'.png', cone_masked_im)

    querys_effect_im = np.concatenate([query_pixel_image[:,:,np.newaxis], querys_effect[:,:,np.newaxis], querys_effect[:,:,np.newaxis]], axis=2)
    plt.imsave(str(Path.home())+'/masked_CNN_ims/querys_effect_'+str(curr_n_layers)+'.png', querys_effect_im)
pdb.set_trace()
