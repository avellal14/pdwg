...
class DataLoader:
	def __init__(self, batch_size, time_steps, data_type='regular', b_context=True, cuda=False):
		...
		if data_type == 'regular':
			self.dataset_path = '~/datasets/mnist/dataset/'
		elif data_type == 'binary':
			self.dataset_path = '~/datasets/mnist/dataset_binary/'
		elif data_type == 'rotation':
			self.dataset_path = '~/datasets/mnist/dataset_rotation/'
	...
	def colorify(self):
		self._batch_obs = self._batch_obs*(0.3+0.7*np.random.uniform(size=(self.batch_size*self.time_steps, 1, 3)))+\
						  (1-self._batch_obs)*(0.3+0.7*np.random.uniform(size=(self.batch_size*self.time_steps, 1, 3)))
	...
	def __next__(self):
		...
		mask_obs, context = self.curr_loader.next_batch(self.batch_size*self.time_steps, shuffle=True)
		self._batch_obs[:,:,0][:] = mask_obs
		self._batch_obs[:,:,1][:] = mask_obs
		self._batch_obs[:,:,2][:] = mask_obs
		self.colorify()
		# self.adjust_range()
		self.noisify()
